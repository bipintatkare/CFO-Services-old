...
import { postBackend } from '../../../helpers/backend';
...
function* loginUser({ payload: { user, history } }) {
    try {
        if (process.env.REACT_APP_DEFAULTAUTH === "firebase") {
            ...
            const response = yield call(postFakeLogin, '/post-fake-login', { username: user.email, password: user.password });
            localStorage.setItem("authUser", JSON.stringify(response));
            yield put(loginSuccess(response));
        }
        else if (process.env.REACT_APP_DEFAULTAUTH === "backend") {
            const response = yield call(postBackend, '/api/v1/login', { username: user.email, password: user.password });
            localStorage.setItem("authUser", JSON.stringify(response));
            yield put(loginSuccess(response));
        }
        history.push('/mom-main');
    } catch (error) {
